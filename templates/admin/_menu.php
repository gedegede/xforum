...
$template_auditPendingStats = $template_adminPermissions['threads'] ? \Models\AuditModel::getPendingStats() : [];
$template_auditPendingTotal = (int)($template_auditPendingStats['pending_threads'] ?? 0) + (int)($template_auditPendingStats['pending_posts'] ?? 0) + (int)($template_auditPendingStats['pending_reports'] ?? 0);
?>
...
<div class="card card-lg mb-4 card-clip">
    <nav class="pill-nav" aria-label="后台菜单">
        <div class="pill-nav-list">
            ...
            <?php if ($template_adminPermissions['threads']): ?><a href="index.php?c=admin&a=audits&filter=all" class="pill-nav-item <?php echo $template_isAuditAdmin ? 'active' : ''; ?>">内容审核<?php echo $template_auditPendingTotal > 0 ? ' ' . $template_auditPendingTotal : ''; ?></a><?php endif; ?>
