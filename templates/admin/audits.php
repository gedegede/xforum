    ...
    <div class="card-body">
        <div class="pill-nav mb-4">
            <div class="pill-nav-list">
                <a href="index.php?c=admin&a=audits&filter=thread" class="pill-nav-item <?php echo $template_filter === 'thread' ? 'active' : ''; ?>">待审主题</a>
                <a href="index.php?c=admin&a=audits&filter=post" class="pill-nav-item <?php echo $template_filter === 'post' ? 'active' : ''; ?>">待审回帖</a>
                <a href="index.php?c=admin&a=audits&filter=report" class="pill-nav-item <?php echo $template_filter === 'report' ? 'active' : ''; ?>">待处理举报</a>
                    ...
                    <div class="audit-item list-link" data-did="<?php echo (int)$audit['did']; ?>">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-1">
                                <span class="badge badge-xs badge-primary"><?php echo htmlspecialchars($audit['type']); ?></span>
                            ...
                            <?php endif; ?>
                            <?php if ((int)$audit['status'] !== 0): ?>
                                <div class="text-xs text-muted mb-1">
                                    处理人：<?php echo htmlspecialchars($auditUser['username'] ?? '未知用户'); ?> · <?php echo (int)$audit['status'] === 1 ? '通过' : '拒绝'; ?> · <?php echo !empty($jsonData['audit_time']) ? \Lib\Helper::formatTime((int)$jsonData['audit_time']) : '-'; ?>
                                </div>
                            <?php endif; ?>
